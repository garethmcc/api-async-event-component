...
const packageExpress = async (instance, inputs) => {
  ...
  instance.state.createHandler = await instance.addSDK(sourceDirectory, '_src/createEntity.createEntity')
  instance.state.deleteHandler = await instance.addSDK(sourceDirectory, '_src/deleteEntity.deleteEntity')
  instance.state.updateHandler = await instance.addSDK(sourceDirectory, '_src/updateEntity.updateEntity')
  instance.state.getHandler = await instance.addSDK(sourceDirectory, '_src/getEntity.getEntity')
  instance.state.getListHandler = await instance.addSDK(sourceDirectory, '_src/getEntityList.getEntityList')
  // instance.state.handler = await instance.addSDK(sourceDirectory, '_src/handler.handler');
...
const createLambda = async (instance, inputs, clients, retries = 0) => {
  ...
  try {
    ...
    instance.state[`${instance.state.currentHandler}-Arn`] = res.FunctionArn;
    instance.state[`${instance.state.currentHandler}-Version`] = res.Version;
  } catch (e) {
...
const createOrUpdateLambda = async (instance, inputs, clients) => {
  ...
  for (const lambdaHandler of lambdaArray) {
    const lambdaArn = `${lambdaHandler}-Arn`

    if (instance.state[lambdaArn] && instance.state[lambdaHandler]) {
      console.log(
        `Verifying the AWS Lambda with the ARN: ${instance.state[lambdaArn]} and Name: ${instance.state[lambdaHandler]} found in state exists...`
      );
      instance.state[lambdaArn] = await getLambda(clients, instance.state[lambdaHandler]);
    }
    if (!instance.state.lambdaArn) {
      instance.state[lambdaHandler] = `${lambdaHandler}-function`; // WARNING: DO NOT ADJUST THIS, OR EVERYONE WILL DEPLOY NEW FUNCTIONS, RATHER THAN UPDATE THEIR OLD ONES.  ADJUST THIS ONLY WHEN WE'RE READY TO DO A BREAKING CHANGE.
      console.log(
        `No AWS Lambda function found.  Creating one with the name: ${instance.state[lambdaHandler]}`
      );
      createLambda({
        state: {
          lambdaName: instance.state[lambdaHandler],
          handler: instance.state[lambdaHandler],
          userRoleArn: instance.state.userRoleArn,
          defaultLambdaRoleArn: instance.state.userRoleArn,
          zipPath: instance.state.zipPath,
          currentHandler: lambdaHandler
        }
      }, inputs, clients);
    } else {
      ...
      console.log(`AWS Lambda version ${lambdaHandler}-Version published`);
      console.log(`AWS Lambda function updated with ARN: ${lambdaHandler}-Arn`);
    }
  }
  return null
  //
...
const addPermission = async (instance, inputs, clients) => {
  ...
  for (const lambdaHandler of lambdaArray) {
    const lambdaArn = instance.state`${lambdaHandler}-aliasArn`;
...
const createOrUpdateApi = async (instance, inputs, clients) => {
  ...
  instance.state.apiGatewayUrl = `https://${instance.state.apiId}.execute-api.${instance.state.region}.amazonaws.com`;
  const lambdaArray = {
    createEntity: `POST /${inputs.version}/${inputs.entityName}`,
    deleteEntity: `DELETE /${inputs.version}/${inputs.entityName}/{id}`,
    getEntity: `GET /${inputs.version}/${inputs.entityName}/{id}`,
    getEntityList: `GET /${inputs.version}/${inputs.entityName}`,
    updateEntity: `POST /${inputs.version}/${inputs.entityName}/{id}`
  }
  for (const [lambdaHandler, routeKey] of lambdaArray) {
    const lambdaArn = instance.state[`${lambdaHandler}-Arn`]
    clients.apig.createRoute({
      ApiId: instance.state.apiId,
      RouteKey: routeKey,
...
const createOrUpdateAlias = async (instance, inputs, clients) => {
  ...
  for (const lambdaHandler of lambdaArray) {
    ...
    if (inputs.traffic && Number(instance.state[`${lambdaHandler}-Version`]) > 1) {
      inputs.alias = 'feature';
    }

    console.log(`Verifying alias "${inputs.alias}"...`);
    instance.state[`${lambdaHandler}-aliasArn`] = await getAlias({
      state: {
        lambdaName: `${lambdaHandler}-function`
      }
    }, inputs, clients);

    const aliasParams = {
      FunctionName: instance.state[`${lambdaHandler}-function`],
      Name: inputs.alias,
      FunctionVersion: instance.state[`${lambdaHandler}-Version`],
    };

    const userDefinedRoutingConfig = await getRoutingConfig({
      state: {
        lambdaName: instance.state[`${lambdaHandler}-function`]
      }
    }, inputs, clients);
    ...
    if (instance.state[`${lambdaHandler}-aliasArn`]) {
      console.log(`Alias "${inputs.alias}" found. Updating...`);
      instance.state[`${lambdaHandler}-aliasArn`] = (await clients.lambda.updateAlias(aliasParams).promise()).AliasArn;
      console.log(`Alias "${inputs.alias}" updated.`);
    } else {
      console.log(`Alias "${inputs.alias}" not found. Creating...`);
      instance.state[`${lambdaHandler}-aliasArn`] = (await clients.lambda.createAlias(aliasParams).promise()).AliasArn;
